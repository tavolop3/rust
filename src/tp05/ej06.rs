...
impl Sistema {
    ...
    fn recibir_criptomoneda(
        ...
        let fiat_equivalente = monto * cotizacion;
        ...
        let transaccion = Transaccion::new(
            Fecha::fecha_actual(),
            TipoTransaccion::RecepcionCripto {
                cripto: criptomoneda.prefijo.clone(),
                blockchain: blockchain.prefijo.clone(),
                monto,
                cotizacion,
...
mod test {
    ...
    fn test_criptomoneda_mas_cantidad_ventas_exitoso() {
        ...
        s.vender_criptomoneda(&xmr, 0.5, &u1).unwrap();
        s.vender_criptomoneda(&xmr, 0.5, &u1).unwrap();
        ...
        assert_eq!(s.cripto_mas_cantidad_ventas(), Some("XMR".to_string()));
    }
    ...
    fn test_criptomoneda_mayor_volumen_ventas_exitoso() {
        ...
        s.buscar_usuario("a@a.com".to_string())
            ...
            .balance_criptos
            .insert("BTC".to_string(), 1.0);
        s.vender_criptomoneda(&btc, 1.0, &u1).unwrap();

        assert_eq!(s.cripto_mas_volumen_ventas(), Some("XMR".to_string()));
    }
